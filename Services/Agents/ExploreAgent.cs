    ...
    public class ExploreAgent : BaseAgent
    {
        ...
        private static string BuildSystemPrompt()
        {
            return CommonSystemPromptPrefix + "\n" +
                ...
                "- 🚫 **严禁重复读取文件**：如果 read_file 返回「已缓存，请勿重复读取」，说明该文件已被完整读取，" +
                "你已拥有其全部内容。**不得再次调用 read_file 读取同一文件**，直接使用已有内容即可。\n" +
                "  重复读取同一文件是严重的 token 浪费，会降低效率并导致你的输出被截断。\n\n" +
                "## 强制探索流程\n" +
                "每轮必须执行以下循环，直到获取充分信息：\n" +
                "1. **list_dir** — 从工作区根目录开始，了解目录结构\n" +
                "2. **file_search** / **grep_search** — 定位相关文件\n" +
                "3. **read_file** — 读取**尚未读过**的关键文件的实际内容\n" +
                "4. 基于实际读取的内容进行分析，然后继续探索**新的**相关文件\n" +
                "5. 重复上述步骤，但**始终跳过已读文件**——每到一个新文件时先判断是否已读过\n\n" +
                "## 网页链接处理\n" +
